...
def dqn(env, agent, n_episodes=2000, max_t=1000, eps_start=1.0, eps_end=0.01, eps_decay=0.995):
    ...
    for i_episode in range(1, n_episodes + 1):
        ...
        print_update(i_episode, mean_score, e_duration)
        if i_episode % 100 == 0:
            print_update(i_episode, mean_score, e_duration, newline=True)
        if mean_score >= 13.0:
...
def main():
    ...
    unity_env = UnityEnvironment(file_name="unity/Banana_Windows_x86_64/Banana.exe", no_graphics=True)
